...
class SDG15_1_1(SDGBase):
    ...
    def _get_file_by_year(
        self, inp_list: List[str], year: int, n: int
    ) -> Optional[List[str]]:
    ...
    def calculate_multiple_years(
        self, year_start: int, year_end: int, n: int = 1, save_shp_file: bool = False
    ) -> bool:
        ...
        for year in tqdm(self._lists_by_year.keys(), "Calculating SDG"):
            ...
            self.calculate_sdg(
                lad_file_path, sam_file_path, nfi_file_path, year, save_shp_file
            )
    ...
    def calculate_sdg(
        ...
        sam_file_path: Optional[str],
        nfi_file_path: Optional[str],
        year: int,
        ...
        shared_cols: List[str] = [
            col for col in lad_gdf.columns if col in sam_df.columns
        ]
        lad_sam_gdf: gpd.GeoDataFrame = lad_gdf.merge(sam_df, on=shared_cols)

        lad_sam_woodland_gdf: gpd.GeoDataFrame = gpd.sjoin(
            lad_sam_gdf, woodland, how="left"
        )

        lad_sam_woodland_gdf[f"pct_woodland_{year}"] = (
            lad_sam_woodland_gdf["area_ha"] / lad_sam_woodland_gdf["arealhect"] * 100
        )

        pct_groupby: gpd.GeoDataFrame = lad_sam_woodland_gdf.groupby(important_col)[
            [f"pct_woodland_{year}"]
        ].sum()
        ...
        lad_gdf.plot(
            column=f"pct_woodland_{year}", figsize=(20, 20), cmap="YlGn", legend=True
        )
...
def run_sdg15_1_1(params: UserParams) -> None:
    ...
    if params.single_year_test and all(
        [
            ...
            params.sam_file_path,
            params.nfi_file_path,
            params.year_start,
        ]
    ):
        ...
        gfr.calculate_sdg(
            ...
            params.nfi_file_path,
            params.year_start,
            save_shp_file=params.save_shp_file,
        )

    if not params.single_year_test and all([params.year_start, params.year_end]):
        print(
            f"Running multi year export for years: {params.year_start}-{params.year_end}"
        )
        gfr.calculate_multiple_years(
            params.year_start, params.year_end, save_shp_file=params.save_shp_file
        )
